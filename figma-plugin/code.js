...
figma.ui.onmessage = async (message) => {
  try {
    ...
    if (message.command === "updateSlide") {
      const result = await updateSlide(message.payload);
      figma.ui.postMessage({ id: message.id, ok: true, result });
      return;
    }

    if (message.command === "exportFrames") {
      const result = await exportFrames(message.payload);
      figma.ui.postMessage({ id: message.id, ok: true, result });
      return;
    }
...
function applySlideContent(frame, slide, index, theme) {
...
function createText(content, fontSize, color, style) {
